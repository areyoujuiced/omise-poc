...
// Prices shown here are cosmetic only — the server holds its own copy and
// re-derives the amount from packageId on every charge, so nothing here is
// trusted for the actual charge.
const PACKAGES = {
  single: { label: 'Single Session', amountBaht: 2600 },
  pack4: { label: '4-Lesson Pack', amountBaht: 9000 },
  pack8: { label: '8-Lesson Pack', amountBaht: 16500 },
};
...
async function init() {
  const res = await fetch('/api/coaching/config');
...
// --- Screen: packages -> method ---
document.querySelectorAll('.package-btn').forEach((btn) => {
  btn.addEventListener('click', () => {
    currentPackageId = btn.dataset.package;
    currentAmountBaht = PACKAGES[currentPackageId].amountBaht;
    document.getElementById('method-amount-preview').textContent =
      `${PACKAGES[currentPackageId].label} — ${formatBaht(currentAmountBaht)}`;
...
document.querySelectorAll('.bank-btn').forEach((btn) => {
  btn.addEventListener('click', async () => {
    ...
    try {
      const res = await fetch('/api/coaching/charge-mobilebanking', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ packageId: currentPackageId, bank, returnUri: window.location.href }),
      });
      ...
      if (authorizeUri) {
        ...
        setStatus('mobilebanking-status', 'Waiting for you to confirm in your banking app…', 'pending');
      } else {
        setStatus('mobilebanking-status', `Waiting for confirmation — ${data.id}`, 'pending');
      }

      pollInterval = setInterval(async () => {
        const statusRes = await fetch(`/api/coaching/charge-status/${data.id}`);
...
cardForm.addEventListener('submit', (e) => {
  ...
  Omise.createToken('card', {
    ...
    try {
      const res = await fetch('/api/coaching/charge-card', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ token: response.id, packageId: currentPackageId }),
      });
      const data = await res.json();
      if (data.object === 'error') {
...
async function startQrCharge() {
  ...
  try {
    const res = await fetch('/api/coaching/charge-qr', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
